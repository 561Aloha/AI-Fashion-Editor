...
var virtual_tryon_exports = {};
__export(virtual_tryon_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(virtual_tryon_exports);
...
// netlify/functions/virtual-tryon.ts
function toDataUrl(input, fallbackMime = "image/png") {
  if (!input) return "";
...
function buildGarmentDes(userPrompt) {
...
async function getClient() {
  ...
  cachedClient = await Client.connect("yisol/IDM-VTON", {
    headers: token ? { Authorization: `Bearer ${token}`, "X-HF-Token": token } : void 0
  });
  return cachedClient;
}
...
var handler = async (event) => {
  try {
    ...
    const imgRes = await withTimeout(fetch(outUrl), 3e4, "Downloading output image");
    if (!imgRes.ok) {
      return {
        statusCode: 500,
...
//# sourceMappingURL=virtual-tryon.js.map
