...
module.exports = __toCommonJS(remove_bg_exports);
...
// netlify/functions/remove-bg.ts
var SPACE_API_URL = "https://briaai-bria-rmbg-1-4.hf.space/--replicas/9gtsr/";
var SPACE_BASE_URL = "https://briaai-bria-rmbg-1-4.hf.space";
  ...
  try {
    ...
    if (typeof out === "string") {
      ...
      if (typeof out.url === "string" && out.url) {
        imageUrl = out.url;
      } else if (typeof out.path === "string" && out.path) {
        const path = out.path;
        imageUrl = `${SPACE_BASE_URL}/file=${encodeURIComponent(path)}`;
        console.log("[remove-bg] Built file URL from path:", imageUrl);
      }
      if (!imageUrl) {
        const msg = "Space returned an object with no usable url/path: " + JSON.stringify(out);
    ...
    const responseBody = { processedBase64 };
    return {
      statusCode: 200,
      headers: {
        "Content-Type": "application/json",
