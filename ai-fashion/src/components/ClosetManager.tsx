...
import type { ImageFile, ClosetItem, Base64Image, FavoriteCreation } from '../types';
...
export const AIStudio: React.FC<AIStudioProps> = ({ modelImage, setModelImage }) => {
    ...
    const handleGenerate = useCallback(async () => {
        ...
        console.log('[DEBUG: AIStudio] Generation parameters:', { modelImage: modelImage[0]?.file.name, outfitImage: outfitImage[0]?.file.name, prompt });
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            const basePrompt = "Take the complete outfit from the second image (the clothing image) and realistically place it on the person from the first image (the model image). Preserve the model's pose and face. The background should be simple or match the model's original background.";
            const finalPrompt = prompt.trim() ? `${basePrompt} Additional instructions: ${prompt}` : basePrompt;

            const resultB64 = await generateFashionImage(modelB64.base64, clothingImages, finalPrompt);
            const imageSrc = `data:image/png;base64,${resultB64}`;
            setGeneratedImage(imageSrc);
            console.log('[DEBUG: AIStudio] Image generation successful. Result snippet:', imageSrc.substring(0, 50) + '...');
    ...
    return (
        <div className="w-full text-gray-800">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* Left Column: The model image uploader, loading state, or final generated image */}
                <div className="space-y-4">
                    {renderMainContent()}
                </div>

                {/* Right Column: Inputs for the outfit image and prompt */}
                <div className="space-y-6 flex flex-col">
                    ...
                    <button
                        ...
                        disabled={isLoading || modelImage.length === 0 || outfitImage.length === 0}
...
// =================================================================
// ClosetManager Component (The new browse/add view)
// =================================================================
...
export const ClosetManager: React.FC<ClosetManagerProps> = ({ closet, setCloset, userId }) => {
    ...
    const { tops, bottoms, dresses, shoes } = useMemo(() => {
        return {
            ...
            bottoms: closet.filter(i => i.category === 'bottoms'),
    ...
    const handleSaveToCloset = (newItem: Omit<ClosetItem, 'id'>) => {
    ...
    return (
        <div className="w-full text-gray-800 relative">
            <div className="space-y-6">
                <AddToCloset onSave={handleSaveToCloset} />

                <div className="mt-8 pt-6 border-t-2 border-gray-300">
                    <h3 className="text-xl font-bold text-gray-800 mb-4 text-center">Browse Your Closet</h3>
                    <div className="w-full max-w-xs mx-auto space-y-4">
                        {tops.length > 0 && (
                            <ClothingCarousel
                                title="Tops"
                                items={tops.map(i => ({ source: i.imageB64 }))}
                        ...
                        {bottoms.length > 0 && (
                            <ClothingCarousel
                                title="Bottoms"
                                items={bottoms.map(i => ({ source: i.imageB64 }))}
                        ...
                        {dresses.length > 0 && (
                            <ClothingCarousel
                                title="Dresses"
                                items={dresses.map(i => ({ source: i.imageB64 }))}
                         ...
                         {shoes.length > 0 && (
                            <ClothingCarousel
                                title="Shoes"
                                items={shoes.map(i => ({ source: i.imageB64 }))}
                         ...
                         {closet.length === 0 && (
                            <div className="flex items-center justify-center h-48 bg-gray-200/50 rounded-md p-4 text-center">
                                <p className="text-gray-500">Your closet is empty. <br/> Add items above to start browsing!</p>
                            </div>
                         )}
                    </div>
                </div>
