...
export default function App() {
        ...
        const loadData = async () => {
            try {
                ...
                if (snapshot.exists()) {
                    ...
                    if (data.modelImage) {
                        const imageFile = base64ToImageFile(data.modelImage);
    ...
    // Save closet
    useEffect(() => {
        if (!user || closet.length === 0) return;
        
        const saveCloset = async () => {
            try {
                const userRef = doc(db, 'users', user.uid);
                await setDoc(userRef, { closet }, { merge: true });
                console.log('[DEBUG: App] Saved closet to Firebase');
            } catch (error) {
                console.error('[DEBUG: App] Error saving closet:', error);
            }
        };
        saveCloset();
    }, [closet, user]);
                ...
                <header className="app-header">
                    ...
                    <div className="header-right">
                        ...
                        <button 
                            ...
                            className="btn-back btn-signout"
                        >
                            Sign Out
                        </button>
                    </div>
